...
def stats_read(zs_samples, samples): # Read N samples of voltage data, do indicative stats.
...
def variance(x, zs_samples,samples):
    ...
    mean1 = np.mean(data1)
    variance1 = np.sum((data1 - mean1)**2)/ len(data1) 
    
    mean2 = np.mean(data2)
    variance2 = np.sum((data2 - mean2)**2)/ len(data2)
    print(variance2-variance1, variance2/variance1)
